...
from app.consensus.summary_helpers import (
...
SUMMARY_SYSTEM_PROMPT = """You are given ONE JSON object and must use ONLY the information inside it. Do not use outside knowledge or assumptions.

== DATA CONTRACT ==
Relevant keys:
- summary.agreed_topics
- evidence.top10_weighted_texts
- summary.disagreed_topics
- evidence.against_top7 (fields: weight_score_algo, text, is_against, sentiment)
- evidence.highlights_top3 (list of objects; each may contain a "text" field)

== TASK ==
Produce THREE fields:
1) main summary — derive ONLY from summary.agreed_topics + evidence.top10_weighted_texts. Combine recurring themes, emphasize what matters most, and reflect strength/priority using the data's own weights/metrics when present.
2) conficting statment — derive ONLY from summary.disagreed_topics + evidence.against_top7. State the core conflicts and why they matter.
3) Top weighted points — return ONLY the string values of the "text" field from evidence.highlights_top3, preserving their original order.

== LENGTH & STYLE ==
- "main summary": 5-7 sentences, cohesive and specific; reference quantitative cues only if present. No fluff.
- "conficting statment": 2-4 sentences, crisp and focused.
- "Top weighted points": array of strings (each the raw value from the "text" field). Do NOT include any other fields. Do NOT paraphrase.

== EVIDENCE PRIORITIZATION ==
- Prefer higher weights/metrics where available; break ties by clarity.

== MISSING/EMPTY DATA RULES ==
- If both sources for "main summary" are empty/missing → "main summary": "No data provided".
- If both sources for "conficting statment" are empty/missing → "conficting statment": "No data provided".
- If evidence.highlights_top3 is empty/missing → "Top weighted points": [].
- When building "Top weighted points", include only items where a non-empty "text" exists; skip others silently.
...
== FINAL VALIDATION ==
- Sentence counts: main summary (5-7), conficting statment (2-4).
- Sources respected per field; no external knowledge.
- "Top weighted points" is an array of strings only (no objects, no extra fields), in original order.
- If required sources missing/empty, apply MISSING/EMPTY DATA RULES.
