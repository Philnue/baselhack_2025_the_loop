...
export default function DiscussionInvitePage({
  params,
}: {
  ...
  const submissionStorageKey = React.useMemo(
    () => `discussion-submitted-${slug}-${userId}`,
    [slug, userId]
  );
          ...
          {discussion.tags.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {discussion.tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center rounded-full border border-pink-400 px-3 py-1 text-sm font-medium text-pink-700"
                >
                  #{tag}
                </span>
              ))}
        ...
        <section className="rounded-xl border border-border bg-card shadow-sm">
          ...
          <div className="space-y-6 px-6 py-6">
            {!hasSubmitted ? (
              ...
              <p className="text-sm text-muted-foreground">
                You have already shared your perspective for this discussion.
              </p>
            )}
